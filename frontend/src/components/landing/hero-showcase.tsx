...
export function HeroShowcase() {
  ...
  useEffect(() => {
    ...
    const play = () => {
      ...
      later(() => setThinking(true), tThink);
      const tEv = tThink + 950;
      s.ev.forEach((_, i) => later(() => setEvN(i + 1), tEv + i * 280));
  ...
  return (
    <div className="relative mx-auto h-[460px] w-full max-w-[560px] sm:h-[500px] [perspective:1100px]">
      ...
      >
        {/* the product window */}
        <div className="absolute left-1/2 top-1/2 w-[86%] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-white/[.14] bg-[#0E0D14]/95 shadow-[0_50px_120px_rgba(0,0,0,.65),inset_0_1px_0_rgba(255,255,255,.12)] backdrop-blur-xl [transform:translateZ(30px)]">
          ...
          <div className="p-5">
            ...
            <div className="mt-3 flex min-h-[30px] flex-wrap gap-2">
              ...
              {SCENES[(sceneRef.current + SCENES.length - 1) % SCENES.length].ev
                .slice(0, evN)
                .map((chip) => (
                  <span
                    key={chip}
